...
class CodeGen_MBCSRRowMaj_Matmul:
    ...
    def GenerateCode(self, inst, codeParams, params):
        """
        Generates r x c register-blocked SPMV code for the MBCSR row major data structure
        ...
        code += tab + "for( I = 0, yp = y + d0*incy; I < M; I++, yp += " + str(r) + "*incy ) {" + nl;
        ...
        # generate diagonal block multiply
        simdLen = r // vectorLen;                        # Number of whole SIMD vectors per row
        partialLen = r % inst.SIMDGetVectorLength();    # Number of leftover scalars per row after all SIMD vectors filled
        
        code += tab + "/* Diagonal block multiply */" + nl
        code += tab + "for( I = 0, yp = y + d0*incy; I < M; I++, yp += " + str(r) + "*incy ) {" + nl
        code += tab*2 + "const oski_value_t* xp = x + (d0 + I*" + str(r) + ");" + nl
        ...
        for i in range(0,simdLen):
            ...
            for j in range(0,vectorLen):
                varName += str(i*vectorLen+j)
            xvars.append(varName)
            code += tab*2 + inst.VectorType() + " " + varName + ";" + nl
        
        xPartial ="_x"
        if (partialLen > 0):
            for j in range(simdLen*vectorLen, r):
                xPartial += str(j)
            code += tab*2 + inst.VectorType() + " " + xPartial + ";" + nl
    
        # Load variables with the x-vector
        for i in range(0, simdLen):
            code += tab*2 + xvars[i] + " = " + inst.SIMDLoadMultiple("xp + " + str(i * vectorLen)) + ";" + nl
        if (partialLen > 0):
            code += tab*2 + xPartial + " = " + inst.SIMDLoadPartial("xp + " + str(simdLen * vectorLen), partialLen) + ";" + nl
    
        code += nl
    
        # Do multiply operations
        code += self.GenerateDiagonalMultiply(inst, r, xvars, yvars, partialLen, xPartial, 2)
        ...
        code += tab*2 + "bdiag += " + str(r) + "*" + str(r) + ";" + nl;
        code += tab + "}" + nl
    ...
    def GenerateMultiply(self, inst, r, c, xvars, yvars, partialLen, xPartial, tabs):
        ...
        for i in range(0, r):
            ...
            if (partialLen > 0):
                ...
                loadCode = inst.SIMDLoadPartial("bval + " + bvalOffset, partialLen)
    ...
    def GenerateDiagonalMultiply(self, inst, r, xvars, yvars, partialLen, xPartial, tabs):
        """
        Generates code for the diagonal multiply stage.
        """
        tab = "    "        # indent character
        nl = "\n"           # newline character
        ...
        for i in range(0, r):
            ytarget = yvars[i]      # y-variable being accumulated into
            for j in range(0, simdLen):
                bvalOffset = str(i*r + j*vectorLen)  # memory offset of the b-vector being multiplied
                loadCode = inst.SIMDLoadMultiple("bdiag + " + bvalOffset)
                multiplyCode = inst.SIMDMultiply(xvars[j], loadCode)
                
                code += tab*tabs + ytarget + " = " + inst.SIMDAdd(ytarget, multiplyCode) + ";" + nl
            
            if (partialLen > 0):
                bvalOffset = str(i*r + simdLen*vectorLen)  # memory offset of the b-vector being multiplied
                loadCode = inst.SIMDLoadPartial("bdiag + " + bvalOffset, partialLen)
                multiplyCode = inst.SIMDMultiply(xPartial, loadCode)
